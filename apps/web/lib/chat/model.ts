...
- Instagram: when a lane contentChannel is \`instagram\` (or ig/insta), message drafts generate a caption + square image pack under Content/instagram/ in the vault. Approve & publish posts via Meta Graph API only when INSTAGRAM_ACCESS_TOKEN + JARVIS_PUBLIC_URL + lane instagramUserId are set and resolve_job / Approve reports contentPublish.published. Otherwise tell the operator to copy from the vault pack. Never claim an Instagram post went live without that confirmation. Autopilot does not auto-publish social.
