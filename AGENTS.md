...
- **Ambient wake word** (always-on) is a toggle in the uplink. Default wake phrase is `jarvis` (also accepts “hey/ok/okay/hi jarvis”); editable and saved in `localStorage`. While Ambient is on, the mic watches continuously, pauses during model replies, and auto-opens the uplink when the wake word is heard. This is browser speech recognition — not a dedicated wake-word DSP model — so keep the tab focused in Chrome/Edge.
